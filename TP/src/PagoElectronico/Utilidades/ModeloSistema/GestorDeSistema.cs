...
namespace PagoElectronico.Utilidades.ModeloSistema
{
    class GestorDeSistema
    {
        ...
        private const string STORE_OBTENERSALDO = "GEM4.spObtenerSaldo";
        ...
        private const string STORE_LISTADO1 = "GEM4.spListadoEstadistico1";
      ...
      public static int loginUsuario(string usuario, string contrasena)
        ...
        public static string obtenerSaldo(int clienteID, long numeroCuenta)
        {
            string saldo;
            List<SqlParameter> parametros = new List<SqlParameter>();
            parametros.Add(new SqlParameter("@clienteID", clienteID));
            parametros.Add(new SqlParameter("@cuentaNro", numeroCuenta));
            object resultadoStoreProcedure = ConexionDB.ConexionDB.InvocarStoreProcedure(STORE_OBTENERSALDO, SCALAR, parametros);
            saldo = resultadoStoreProcedure.ToString();
            return saldo;
        }
        ...
        public static DataTable listado1(int anio, int trimestre)
        {
            List<SqlParameter> parametros = new List<SqlParameter>();
            parametros.Add(new SqlParameter("@anio", anio));
            parametros.Add(new SqlParameter("@trimestre", trimestre));
            SqlDataReader readerListado = (SqlDataReader)ConexionDB.ConexionDB.InvocarStoreProcedure(STORE_LISTADO1, READER, parametros);
        ...
        {
            List<SqlParameter> parametros = new List<SqlParameter>();
            SqlDataReader readerClientes = (SqlDataReader)ConexionDB.ConexionDB.InvocarStoreProcedure(STORE_OBTENERCLIENTES, READER, parametros);
            DataTable clientes = new DataTable();
            if (readerClientes.HasRows)
            {
                clientes.Load(readerClientes);
        ...
        public static string facturar(int clienteID)
        {
            string mensajeResultado;
            List<SqlParameter> parametros = new List<SqlParameter>();
            parametros.Add(new SqlParameter("@ClienteID", clienteID));
            object resultadoStoreProcedure = ConexionDB.ConexionDB.InvocarStoreProcedure(STORE_FACTURAR, SCALAR, parametros);
            mensajeResultado = resultadoStoreProcedure.ToString();
            return mensajeResultado;
        }
        ...
        public static bool cuentaGratuitaPendiente(int cliente)
        {
            int existe;
            List<SqlParameter> parametros = new List<SqlParameter>();
